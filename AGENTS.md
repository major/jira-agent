...
| `internal/commands/composite.go` | Composite workflow commands: `issue start-work`, `issue close`, `issue create-and-assign`, `issue create-and-link`, `issue move-to-sprint`, `issue transition-jql`. |
| `internal/commands/sprint.go` | Sprint subcommands: `sprint list`, `sprint get`, `sprint create`, `sprint update`, `sprint delete`, `sprint issues`, `sprint move-issues`, `sprint swap`, `sprint property`, `sprint current`, `sprint summarize`. |
